...
export async function analyzeSymbol(symbol){
  ...
  const analysis=full.mode==='full-eod'?full:{...snapshot,mode:'snapshot-partial',warnings:[full.reason,'النتيجة الحالية تستخدم المؤشرات المختصرة إلى أن يكتمل تاريخ EOD.']};
  return {symbol,companyName:company?.arabicName||STATIC_NAMES[symbol]||market.description,quote:{last:market.close,open:market.open,high:market.high,low:market.low,volume:market.volume,change:market.change,bid:null,ask:null},analysis,history:{storedCandles:Math.max(0,history.length-1),requiredCandles:120,fullEngine:full.mode==='full-eod'},updatedAt:new Date().toISOString(),sources:{group:{url:GROUP_URL,status:groupStatus,usage:'التحقق من الرمز واسم الشركة'},tradingView:{status:'connected',usage:'السعر والمؤشرات الفنية والشارت ولقطة EOD'}}};
}
