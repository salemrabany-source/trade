...
export async function analyzeSymbol(symbol){
  ...
  const full=analyzeEod({daily:history,intraday}),snapshot=buildAnalysis(market);
  ...
  return {symbol,companyName:company?.arabicName||STATIC_NAMES[symbol]||market.description,englishName:company?.englishName||market.description,quote:{last:market.close,open:market.open,high:market.high,low:market.low,volume:market.volume,change:market.change,bid:null,ask:null},analysis,history:{storedCandles:Math.max(0,history.length-1),intradayCandles:intraday.length,requiredCandles:120,requiredIntradayCandles:30,fullEngine:full.mode?.startsWith('full-')},chart:{daily:{interval:'1D',candles:chartCandles},hourly:{interval:'60',candles:intraday.slice(-500)}},updatedAt:new Date().toISOString(),sources:{group:{url:GROUP_URL,status:groupStatus,usage:'التحقق من الرمز واسم الشركة'},tradingView:{status:'connected',usage:'السعر والمؤشرات الفنية ولقطة EOD'}}};
}
