...
export async function analyzeSymbol(symbol){
  ...
  const market=await tradingViewSnapshot(symbol),analysis=buildAnalysis(market),company=catalog[symbol];
  return {symbol,companyName:company?.arabicName||STATIC_NAMES[symbol]||market.description,quote:{last:market.close,open:market.open,high:market.high,low:market.low,volume:market.volume,change:market.change,bid:null,ask:null},analysis,updatedAt:new Date().toISOString(),sources:{group:{url:GROUP_URL,status:groupStatus,usage:'التحقق من الرمز واسم الشركة'},tradingView:{status:'connected',usage:'السعر والمؤشرات الفنية والشارت'}}};
}
