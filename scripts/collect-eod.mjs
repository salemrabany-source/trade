...
const URL='https://scanner.tradingview.com/qatar/scan',columns=['name','time','open','high','low','close','volume'];
...
for(const row of payload.data||[]){const symbol=row.s.split(':')[1],x=Object.fromEntries(columns.map((k,i)=>[k,row.d[i]])),barDate=Number.isFinite(Number(x.time))?new Date(Number(x.time)*1000).toISOString().slice(0,10):null;if(barDate!==date||!['open','high','low','close','volume'].every(k=>Number.isFinite(Number(x[k]))))continue;const path=`data/${symbol}.json`;let old=[];try{old=JSON.parse(await readFile(path,'utf8'))}catch{}const byDate=new Map(old.map(c=>[c.date,c]));byDate.set(date,{date,open:+x.open,high:+x.high,low:+x.low,close:+x.close,volume:+x.volume});await writeFile(path,JSON.stringify([...byDate.values()].sort((a,b)=>a.date.localeCompare(b.date)),null,2)+'\n')}
