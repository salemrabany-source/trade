...
const [csvPath,intervalArg='60',...rawArgs]=process.argv.slice(2),replaceMode=rawArgs.includes('--replace'),mappingArgs=rawArgs.filter(arg=>arg!=='--replace');
...
for(const {prefix,symbol} of mappings){
  ...
  let existing=[];if(!replaceMode)try{existing=JSON.parse(await readFile(outputPath,'utf8'))}catch{}const unique=[...new Map([...existing,...rows].map(c=>[c.date,c])).values()].sort((a,b)=>a.date.localeCompare(b.date));await writeFile(outputPath,`${JSON.stringify(unique,null,2)}\n`,'utf8');results.push({prefix,symbol,interval:dailyMode?'1D':interval,replaceMode,candles:unique.length,imported:rows.length,removedForwardFilled:parsedRows.length-rows.length,first:unique[0]?.date,last:unique.at(-1)?.date,outputPath});
}
